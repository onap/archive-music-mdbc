...
public class MdbcTestMultiClient implements Runnable {
    ...
    private static String[] tableNames = {"persons2", "persons"};
...
//    	executeUpdateTimed(generateStatement(employeeMap, r, tableName), insertStmt, true);
    	...
    	while (firstTry || r.nextBoolean()) {
    	    firstTry = false;
    	    int tableNum = r.nextInt(tableNames.length);
            String tableName = tableNames[tableNum];
            HashMap<Integer, Employee> employeeMap = employeeMaps.get(tableName);
            if (r.nextInt(100)<selectInsteadOfUpdatePct) {
                ...
                executeUpdateTimed(generateStatement(employeeMap, r, tableName, tableNum), insertStmt, true);
            }
    	}
    ...
    private String generateStatement(HashMap<Integer, Employee> employeeMap, Random r, String tableName, int tableNum) {
    	String toRet = null;
    	...
    	if (toRet==null) {
    		toRet = generateInsert(employeeMap, r, tableName, tableNum);
    	}
	...
	private String generateInsert(HashMap<Integer, Employee> employeeMap, Random r, String tableName, int tableNum) {
		String toRet = null;
		...
		while (id==null) {
			...
			id = ((id/tableNames.length) * tableNames.length) + tableNum;
