...
class RemoteMeta extends MetaImpl {
  ...
  @Override public void heartbeat(final ConnectionHandle ch) {
      connection.invokeWithRetries(new CallableWithoutException<Void>() {
        public Void call() {
          final Service.MdbcHeartbeatResponse response =
              service.apply(new Service.MdbcHeartbeatRequest(ch.id));
          return null;
        }
      });
    }
